fn main() {
    let mut pw_counter_vec: Vec<u32> = Vec::new();
    for c in CHAR_CHECK.iter() {
        for p in PASSWORD.iter() {
            let mut counter: u32 = 0;
            let mut p_char_iter = p.chars();

            loop {
                match p_char_iter.next() {
                    Some(p) => {
                        if &p == c {
                            counter += 1;
                            println!("{}", p);
